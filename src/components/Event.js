...
function Event({ poster, title, description, mapsURL, ticketsURL, listenOn }) {
  return (
    ...
    >
      ...
      {mapsURL.length > 0 && (<Link href={mapsURL} target="_blank" rel="noopener noreferrer">
        <Button variant="contained" color="primary">
          View on Google Maps
        </Button>
      </Link>)}

      {ticketsURL.length > 0 && (<Link href={ticketsURL} target="_blank" rel="noopener noreferrer" sx={{ mt: 2 }}>
